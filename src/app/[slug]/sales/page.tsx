...
export default function SalesPage() {
  ...
  const { data: orders, isLoading } = useQuery({
    queryKey: ["sales-orders", slug, page, debouncedSearch, statusFilter],
    queryFn: () =>
      fetch(
        `/api/${slug}/sales-orders?page=${page}&limit=20&search=${debouncedSearch}&status=${statusFilter}`
      ).then((r) => r.json()),
    enabled: !!slug,
  });
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader
        eyebrow="Commerce"
        title="Sales"
        description="Sales orders and the deliveries that fulfil them."
        actions={
          <Link href={`/${slug}/sales/new`}>
            <Button size="lg">
              <Plus className="h-4 w-4" strokeWidth={2} /> New sales order
            </Button>
          </Link>
        }
      />
